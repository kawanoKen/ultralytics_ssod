#!/usr/bin/env bash
# Quick single-GPU throughput sweep to pick a good batch size before the full
# 4-GPU DDP baseline run (train_labeled_baseline.sh). Each (model, batch) is a
# fresh subprocess so an OOM at one size doesn't kill the rest of the sweep.
set -euo pipefail
...
LOG_DIR="scripts/voc_baseline/bench_logs"
mkdir -p "${LOG_DIR}"
