#!/usr/bin/env bash
# Additional seeds for loss-balance + DFL per-edge confidence masking.
set -uo pipefail
...
for seed in 1 2; do
  run "${MODEL_10P}" "${PROJECT_10P}" crowdhuman_10p.yaml "yolov8n_10p_loss_balance_edge_conf_seed${seed}" "${seed}"
  run "${MODEL_1P}"  "${PROJECT_1P}"  crowdhuman_1p.yaml  "yolov8n_1p_loss_balance_edge_conf_seed${seed}"  "${seed}"
done

echo "=== Additional loss-balance + DFL edge-confidence seeds finished. ==="
